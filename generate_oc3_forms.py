#!/usr/bin/env python3
"""
Generate Final OC Presentation evaluation pages and inject pills into root index.
"""
...
def inject_oc3_links_into_capstone_index(filename_map: dict) -> None:
    """Add a Final OC Presentation pill for each student with an OC3 form."""
    ...
    for display, key in display_to_key.items():
        ...
        fn = filename_map[key]
        link = (
            f'<a href="oc3_forms_output/{fn}" '
            f'class="eval-btn oc3-btn">Final OC Presentation</a>'
        )
        pattern = re.compile(
            r'(<span class="student-name">'
            + re.escape(display)
            + r'</span>\s*<div class="eval-btns">)(.*?)(</div>)',
            re.DOTALL,
        )

        def repl(m: re.Match, _link: str = link) -> str:
            prefix, mids, close = m.group(1), m.group(2), m.group(3)
            if "Final OC Presentation" in mids:
                return m.group(0)
...
def main() -> None:
    ...
    inject_oc3_links_into_capstone_index(filename_map)

    print(f"\nDone: {len(generated)} files in {OUTPUT_DIR}/")
    print("Capstone index.html updated with Final OC Presentation pills where data exists.")
